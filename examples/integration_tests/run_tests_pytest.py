...
def test_config():
    ...
    return {
        ...
        "fixed_agent_name_for_export": os.getenv(
...
def test_paths():
    ...
    return {
        ...
        / os.getenv("OUTPUT_DIRECTORY", "test_results"),
...
def import_example_module(example_path: Path) -> object:
    ...
    spec = importlib.util.spec_from_file_location(example_path.stem, str(example_path))
...
def test_create_agent_from_json_file(test_config, test_paths):
    ...
    args = [
        ...
        test_config["password"],
        "--json_file",
        str(test_paths["assets_dir"] / "input.json"),
    ]
...
def test_create_agent_from_modular_directory(test_config, test_paths):
    ...
    args = [
        ...
        test_config["password"],
        "--agent_directory",
        str(test_paths["modular_agent_dir"]),
        "--agent_name",
...
def test_create_agent_from_nested_directory(test_config, test_paths):
    ...
    args = [
        ...
        test_config["password"],
        "--agent_directory",
        str(test_paths["assets_dir"] / "nested_agent_dir"),
        "--agent_name",
        "order_management_agent",
    ]
...
def test_create_apex_class(test_config, test_paths):
    ...
    args = [
        ...
        test_config["password"],
        "--output_dir",
        str(test_paths["results_dir"] / "apex_classes"),
    ]
...
def test_generate_prompt_template(test_config, test_paths):
    ...
    # Create specific directory for this template to avoid conflicts
    template_output_dir = test_paths["results_dir"] / "prompt_templates"
    ...
    try:
        ...
        template_file = (
            template_output_dir / "Account_Health_Analysis_Template.promptTemplate"
        )
        assert template_file.exists(), f"Template file not created at {template_file}"
    finally:
        sys.argv = original_argv


@pytest.mark.dependency()
def test_generate_template_with_apex(test_config, test_paths):
    """Test generating prompt template with Apex actions."""
    if not test_config["security_token"]:
        pytest.skip("Security token not provided")

    example_path = test_paths["examples_dir"] / "generate_template_with_apex_example.py"
    module = import_example_module(example_path)

    # Create specific directory for apex templates
    apex_template_dir = test_paths["results_dir"] / "apex_templates"
    apex_template_dir.mkdir(parents=True, exist_ok=True)
    ...
    try:
        ...
        # Verify the template was created
        template_file = (
            apex_template_dir / "Account_Opportunity_Analysis_Template.promptTemplate"
        )
        assert template_file.exists(), f"Template file not created at {template_file}"
    finally:
        sys.argv = original_argv
...
def test_deploy_prompt_template(test_config, test_paths):
    ...
    template_path = (
        test_paths["results_dir"]
        / "prompt_templates"
        / "Account_Health_Analysis_Template.promptTemplate"
    )
    if not template_path.exists():
        pytest.skip(f"Template file not found at {template_path}")
...
def test_tune_prompt_template(test_config, test_paths):
    ...
    args = [
        ...
        str(template_path),
        "--output_dir",
        str(test_paths["results_dir"] / "tuned_templates"),
        "--model",
        "gpt-4",
    ]
...
# API server example
@pytest.mark.skipif(
    not os.getenv("API_AUTH_TOKEN"), reason="API auth token not provided"
)
...
def test_export_salesforce_agent(test_config, test_paths):
    ...
    args = [
        ...
        test_config["domain"],
        "--agent_name",
        test_config["fixed_agent_name_for_export"],
        "--output_dir",
        str(test_paths["results_dir"] / "exported_agents"),
    ]
